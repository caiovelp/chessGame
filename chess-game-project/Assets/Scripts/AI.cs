...
public class Piece {
    ...
    public Move[] Movement(Board board){
        switch(this.type){
            ...
            case 3: return this.Axis(board); // Castelo   
            case 4: return this.Queen(board); // Rainha    
        }
        return this.Surround(board); // Rei
    }
    ...
    public Move Point(Board b, int x, int y)
    {
        if (b.VerifyInsideBoard(x, y))
        {
            ...
            */ 
            if (piece == null)
            {
                return new Move(this.x, this.y, x, y);
            }
            else if (piece.team != this.team)
            {
                return new Move(this.x, this.y, x, y, piece.TypeToScore());
            }
        }
